...
{
    public partial class ReporterComments : Form
    {
        private int currentUserId;
        string cs = ConfigurationManager.ConnectionStrings["dbcs"].ConnectionString;
        public ReporterComments()
        {
            InitializeComponent();
        }

        public ReporterComments(int userId)
        {
            InitializeComponent();
        ...
        private void LoadReportDetails(int reportId)
        {
            using (SqlConnection con = new SqlConnection(cs))
            {
                string query = @"SELECT Title 
                         FROM Reports 
                         WHERE ReportId = @ReportId AND SubmittedBy = @UserId";
                ...
                {
                    MessageBox.Show("Report not found or you are not authorized to view it.");
